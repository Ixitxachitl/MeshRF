...
public partial class ScriptHelpWindow : Window
{
    ...
    [
        ...
        new("geofence: \"North Gate\"", "A node crosses the fence on the waypoint of that name — including this node, so a fence round home reports you arriving. Use 'any' for whichever fence is crossed. Matched by name, so a waypoint that expires and is dropped again keeps working. The editor offers the fences currently on your map."),
        new("  on: enter", "Which way across: enter (the default), exit, or both. Independent of the waypoint's own notify_on_enter/notify_on_exit, which only govern the chime and the channel note — a script sees every crossing of a fence it names."),
    ...
    [
        ...
        new("from: [\"!a1b2c3d4\"]", "Only these senders. \"{my.id}\" names this node, for the one event it can set off itself — walking into its own geofence."),
        new("not_from: [\"!deadbeef\"]", "Never these senders. not_from: [\"{my.id}\"] is how a script that greets arrivals stops greeting you."),
    ...
    [
        new("reply: \"text\"", "Answer in the conversation the trigger arrived on, threaded under the message. Under a geofence: trigger it posts to the fence's channel, unthreaded, since a crossing is not a message. To answer somewhere else — another channel, or the sender of a channel message by DM — use send: instead."),
        ...
        new("nodeinfo:", "Indent request: true instead to ask the triggering node for ITS NodeInfo. That answer carries their public key, which is the only way a script can reach somebody it has never heard from by DM. Costs a round trip, so put a delay: after it — and gate both with when: value: \"{from.has_key}\" / equals: false so you only ask about somebody you do not already know."),
        new("  request: true", "Ask for theirs rather than advertising ours."),

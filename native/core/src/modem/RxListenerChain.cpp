...
std::span<const Sample> RxListenerChain::process(std::span<const Sample> in) {
    ...
    // The level of this block, measured after the channel filter, handed down
    // before the block itself: a preamble found in it reports what the signal
    // in it was worth, and only the modem knows how much of the block was
    // signal. Read off the receiver later instead, this measures the channel
    // as it is then — and by then the frame has ended.
    const float level_dbfs = stats_.snapshot().rssi_dbfs;
    for (auto& modem : modems_) {
        modem->set_channel_level_dbfs(level_dbfs);
        modem->process_rx(channel);
    }
    return channel;
}
